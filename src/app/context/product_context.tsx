...
type ProductContextType = {
    ...
    uploadImage: (formData: FormData) => Promise<object | undefined>
}
...
const defaultProductContext: ProductContextType = {
    ...
    uploadImage: async (_formData: FormData) => {
        return {};
    }
}
...
export function ProductContextProvider({ children } : PropsWithChildren){
    ...
    async function uploadImage(formData: FormData){
        try {
            const response = await productRepositoryHttp.uploadImage(formData);
            return response;
        } catch (error:any) {
            return error;
        }
